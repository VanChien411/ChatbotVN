...
print('{pathFirstData}')
with open('{pathFirstData}', 'r', encoding='utf-8') as f:
    intents = json.load(f)
...
def createMatrix(pth):
    # Use an absolute path to load the data file
    data_file_path = os.path.join(os.path.dirname(__file__), pth)
...
def saveTrainData(pth, data):
    FILE = os.path.join(current_file_dir, pth)
